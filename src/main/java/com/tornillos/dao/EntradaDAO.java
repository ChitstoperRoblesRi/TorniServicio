...
public class EntradaDAO {
    ...
    public int contarHoy() throws SQLException {
        try (Statement st = DatabaseConfig.getConnection().createStatement();
             ResultSet rs = st.executeQuery("SELECT COUNT(*) FROM entradas WHERE DATE(fecha)=CURRENT_DATE")) {
    ...
    public List<Entrada> buscar(String termino, String desde, String hasta) throws SQLException {
        ...
        StringBuilder sb = new StringBuilder(
            ...
            "FROM entradas e JOIN tornillos t ON e.tornillo_id=t.id " +
        ...
        if (desde != null) { sb.append("AND e.fecha >= (?::date)::timestamp "); params.add(desde); }
        if (hasta != null) { sb.append("AND e.fecha < ((?::date) + 1)::timestamp "); params.add(hasta); }
